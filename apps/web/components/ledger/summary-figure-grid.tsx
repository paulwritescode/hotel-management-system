...
export type FigureTone = 'revenue' | 'waived' | 'unpaid' | 'count' | 'neutral'
export type SummaryFigure = { label: string; value: string; window: string; tone?: FigureTone }
...
export function SummaryFigureGrid({ figures }: { figures: SummaryFigure[] }) {
  return <dl className="summary-grid">
    {figures.map((figure) => <div key={figure.label} className="summary-figure">
      ...
      <dd className={`figure-value${figure.tone ? ` figure-value-${figure.tone}` : ''}`}>{figure.value}</dd>

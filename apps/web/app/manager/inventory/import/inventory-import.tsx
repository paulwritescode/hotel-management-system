...
const fields: Array<{ value: InventoryField | ''; label: string }> = [
  { value: '', label: 'Do not import' }, { value: 'name', label: 'Name' }, { value: 'nameSwahili', label: 'Swahili name' }, { value: 'description', label: 'Description' }, { value: 'category', label: 'Category' }, { value: 'priceKes', label: 'Price in KES' }, { value: 'preparationMinutes', label: 'Preparation time (minutes)' }, { value: 'available', label: 'Available' }, { value: 'quantityOnHand', label: 'Quantity on hand' }, { value: 'unit', label: 'Unit' },
]
...
export function InventoryImport() {
  ...
  async function approve() {
    ...
    const rows: Array<Omit<Item, '_id' | 'archived'>> = validRows.map((row) => {
          const item: Omit<Item, '_id' | 'archived'> = { name: row.name!, category: row.category!, priceKes: row.priceKes!, preparationMinutes: row.preparationMinutes ?? 20, available: row.available ?? true }
  ...
  return <DashboardShell section="Import menu items" actions={<Link className="button button-secondary button-small" href="/manager/inventory">Back to menu items</Link>}><section className="page-section page-section-narrow"><div className="section-heading"><div><p className="caption">CSV and Excel</p><h1>Import menu items</h1><p className="muted">Map once, validate every row, then upsert the valid subset in one transaction</p></div></div><label className="upload-zone"><span><strong>{fileName || 'Choose a spreadsheet'}</strong><br/><span className="caption muted">CSV or XLSX · parsed only in this browser</span></span><input className="sr-only" type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={chooseFile} /></label>
